...
theory Small_Step_Ex7 imports "~~/src/HOL/IMP/Star" Big_Step_Ex7 begin
...
Repeat:  "(REPEAT c UNTIL b, s) \<rightarrow>
            (c ;; IF b THEN SKIP ELSE (REPEAT c UNTIL b), s)"
...
next
  fix b c s t
  let ?if = "IF b THEN SKIP ELSE (REPEAT c UNTIL b)"
  assume c: "(c, s) \<rightarrow>* (SKIP, t)"
  assume b: "bval b t"
  have i: "(?if, t) \<rightarrow>* (SKIP, t)" by (simp add: b)
  have "(REPEAT c UNTIL b, s) \<rightarrow> (c;; ?if, s)" by blast
  moreover have "(c;; ?if, s) \<rightarrow>* (SKIP, t)" by (rule seq_comp[OF c i])
  ultimately show "(REPEAT c UNTIL b, s) \<rightarrow>* (SKIP, t)" by (metis star.simps)
next
  fix b c s s' t
  let ?r = "REPEAT c UNTIL b"
  let ?if = "IF b THEN SKIP ELSE ?r"
  assume c: "(c,s) \<rightarrow>* (SKIP,s')"
  assume b: "\<not>bval b s'"
  assume r: "(?r, s') \<rightarrow>* (SKIP, t)"
  have "(?if,s') \<rightarrow> (?r, s')" by (simp add: b)
  moreover have "(?r, s') \<rightarrow>* (SKIP, t)" by (simp add: r)
  ultimately have i: "(?if,s') \<rightarrow>* (SKIP,t)" by (metis star.simps)
...
next
  case RepeatTrue thus ?case
    by (blast intro: seq_comp star.step)
next
  case RepeatFalse thus ?case
    by (metis star_trans star.step[OF Repeat star_seq2]
      star.step[OF Seq1 star.step[OF IfFalse star.refl[of small_step]]])
qed
